...
Respond with ONLY the category name in lowercase, nothing else."""
    ...
    def export_to_excel(self, output_path):
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
                       SELECT trans_date, description, reference_number, amount, category, ai_classified
                       FROM transactions
                       ORDER BY trans_date
                       """)
        ...
        for trans in transactions:
            date_obj = datetime.strptime(trans[0], '%Y-%m-%d')
        ...
        for month_name, trans_list in sorted(monthly_data.items()):
            ...
            # Headers
            headers = ['Date', 'Description', 'Reference', 'Amount', 'Category']
            ws.append(headers)
            ...
            for trans in trans_list:
                ...
                if trans[5] == 1 and category:  # ai_classified flag
                    category = f"{category} 🤖"

                ws.append([formatted_date, trans[1], trans[2], trans[3], category])

            # Add data validation to category column
            last_row = ws.max_row
        ...
        ai_count = sum(1 for trans in transactions if trans[5] == 1)
        if ai_count > 0:
